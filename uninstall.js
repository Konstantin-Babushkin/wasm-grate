...
const binp = path.join(cargoDir, "bin", "wasm-grate");

if (fs.existsSync(binp)) {
  console.log("Uninstalling wasm-grate...");
  exec(`cargo uninstall wasm-grate`, (error, stdout, stderr) => {
    console.log(stdout);
    if (error || stderr) {
  ...
  console.log("wasm-grate not found skipping!");
}
